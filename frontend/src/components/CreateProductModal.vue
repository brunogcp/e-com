...
export default {
  ...
  data () {
    return {
      ...
      modalTitleLoggedIn: 'Novo Produto!',
      loginBtnLabel: 'Cadastrar',
  ...
  computed: {
    ...
    openModal () {
      if (this.$store.getters.isCreateProductModalOpen) {
        return true;
      } else {
        return false;
      }
  ...
  methods: {
    ...
    closeModal () {
      this.$store.commit('showCreateProductModal', false);
    },
    async checkForm (e) {
      e.preventDefault();
      ...
      if (this.name && this.price) {
        this.highlightNameWithError = false;
        this.highlightPriceWithError = false;
        await this.$store.dispatch('createProduct', { name: this.name, price: parseInt(this.price), description: this.description })
        this.clean()
        this.closeModal()
